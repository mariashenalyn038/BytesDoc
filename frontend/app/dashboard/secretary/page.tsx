...
import { FileText, Archive, Upload, Activity } from 'lucide-react'
...
function SecretaryDashboardContent() {
  ...
  const categoryData = SECRETARY_CATEGORIES.map(c => ({
    name: c,
        ...
        <div className="px-6 pt-5 pb-8 space-y-6">
          <div>
            ...
            <p className="text-[13px] text-gray-500 dark:text-gray-400 mt-1">
              Manage proposals, permits, and reports for the{' '}
              <span className="font-semibold text-gray-700 dark:text-gray-300">{currentAdmin}</span> administration.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <Card title="My uploads" value={myUploads} icon={<Upload size={22} />} accent="violet" />
            <Card title="Active documents" value={totalDocs} icon={<FileText size={22} />} accent="blue" />
          ...
          <div className="bg-white dark:bg-white/[0.02] ring-1 ring-border-subtle dark:ring-white/5 shadow-soft rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                onClick={() => router.push('/dashboard/secretary?tab=documents')}
